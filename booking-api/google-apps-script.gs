...
function doGet(e) {
  ...
  return reply(route(actionOf(e, p), p), p.callback);
}

/* The widget falls back to a form-encoded sendBeacon POST when the JSONP
 * <script> path is blocked or served a non-JS page, so POST must route on
 * `action` exactly as GET does: letting an unrouted POST default to book()
 * would turn a failure report into a booking attempt. The legacy default
 * survives only for a JSON body, which is what the pre-JSONP widget sent. */
function doPost(e) {
  e = e || {}; var p = e.parameter || {};
  var raw = (e.postData && e.postData.contents) || "";
  var isJson = raw.charAt(0) === "{";
  var body = p;
  if (isJson) { try { body = JSON.parse(raw); } catch (err) { body = p; } }
  if (!body.action && p.action) body.action = p.action;
  return reply(route(actionOf(e, body) || (isJson ? "book" : ""), body), p.callback || body.callback);
}

function actionOf(e, p) {
  return p.action || (e.pathInfo || "").replace(/^\//, "");
}

/* One routing table for both verbs. Every handler is wrapped: an uncaught
 * throw would make Apps Script serve an HTML error page, which a JSONP
 * <script> load cannot read — the widget would see silence, not the reason. */
function route(action, p) {
  if (action === "availability") {
    try { return availability(p); }
    catch (err) {
      notifyIssueSafe("availability", err, p);
      return { ok: false, error: safeError(err), slots: [] };
    }
  }
  if (action === "book") {
    try { return book(p); }
  ...
  if (action === "issue") return notifyIssueFromClient(p);
  return {
    ok: true,
    service: "SF Booking (Google + iCloud)",
    endpoints: ["availability", "book", "health", "issue"]
  };
}
...
function health() {
  ...
  /* Booking sends invites and every alert goes out by mail, so a revoked mail
   * scope or an exhausted quota breaks both while availability keeps working —
   * the failure that looks like the service ignoring you. Reading the quota
   * needs the same scope as sending, so this proves both at once. */
  try {
    out.mail.remainingQuota = MailApp.getRemainingDailyQuota();
    if (out.mail.remainingQuota <= 0) {
      out.mail.ok = false;
      out.mail.error = "Daily mail quota exhausted — invites and alerts cannot be sent.";
      out.ok = false;
    }
  } catch (err) {
    out.mail.ok = false;
    out.mail.error = safeError(err);
    out.ok = false;
  }
